                        ...
                        <tbody>
                            @foreach ($prospects as $prospect)
                                <tr>
                                    ...
                                    <td>
                                        <span
                                            class="badge {{ $prospect->status === 'new' ? 'bg-primary' : ($prospect->status === 'interested' ? 'bg-success' : ($prospect->status === 'not interested' ? 'bg-danger' : '')) }} px-2 py-2">
                                            {{ $prospect->status }}
                                        </span>
                                    </td>
                                    ...
                                    <td>
                                        <div class="dropdown d-inline-block">
                                            @can('view prospects')
                                                {{-- show --}}
                                                <a href="{{ route('prospects.show', $prospect->id) }}" type="button"
                                            ...
                                            @can('view prospects')
                                                {{-- edit --}}
                                                <a href="{{ route('prospects.edit', $prospect->id) }}" type="button"
                                            ...
                                            @can('delete prospects')
                                                ...
                                                <button class="btn btn-danger waves-effect waves-light" data-bs-toggle="modal"
                                                    data-bs-target="#deleteRecordModal{{ $prospect->id }}" title="Supprimer">
                                                    <i class="ri-delete-bin-line"></i>
                                                </button>

                                                {{-- delete modal --}}
                                                <div class="modal fade zoomIn" id="deleteRecordModal{{ $prospect->id }}"
                                            ...
                                            @can('manage status prospects')
                                                {{-- status button --}}
                                                <button class="btn btn-success waves-effect waves-light" data-bs-toggle="modal"
                                                    data-bs-target="#statusModal{{ $prospect->id }}" title="Statut">
                                                    <i class="ri-dice-4-line"></i>
                                                </button>

                                                {{-- status modal --}}
                                                <div id="statusModal{{ $prospect->id }}" class="modal zoomIn" tabindex="-1"
                                                    ...
                                                    <div class="modal-dialog modal-dialog-centered">
                                                        <div class="modal-content border-0 overflow-hidden">
                                                            ...
                                                            <div class="modal-body">
                                                                <form action="{{ route('prospects.status', $prospect->id) }}"
                                                                    method="POST">
                                                                    @csrf
                                                                    @method('PUT')
                                                                    ...
                                                                    <div class="mb-3">
                                                                        ...
                                                                        <select name="status"
                                                                            ...
                                                                            <option value="not interested">Pas intéressé
                                                                            </option>
                                                                        </select>
                                                                    </div>
